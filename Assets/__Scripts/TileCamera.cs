...
public class TileSwap
{
    ...
    public GameObject guarantiedItemDrop;
...
public class TileCamera : MonoBehaviour
{
    ...
    static public Transform TILE_ANCHORS;
    ...
    /// <summary>
    /// Generates all tiles of the map at once
    /// </summary>
        ...
        for (int j = 0; j < H; j++)
        {
            for (int i = 0; i < W; i++)
            {
                if (MAP[i,j] != 0)
                {
                    ...
                    ti.transform.SetParent(TILE_ANCHORS);
                    ti.SetTile(i, j);
                    TILES[i,j] = ti;
                }
            }
    ...
    private void CheckTileSwaps(int i, int j)
    {
        ...
        if (ts.swapPrefab != null)
        {
            ...
            if (ts.guarantiedItemDrop != null)
            {
                if (e != null)
                {
                    e.guarantiedItemDrop = ts.guarantiedItemDrop;
                }
            }
